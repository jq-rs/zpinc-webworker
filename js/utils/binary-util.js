const BinaryUtil = {
  ...
  /**
   * Constant-time conditional select between two Uint8Arrays.
   * Returns a new array whose bytes come from `a` where flag is true,
   * or from `b` where flag is false, without branching on the flag value.
   * @param {boolean} flag - Selection flag (true picks a, false picks b)
   * @param {Uint8Array} a - Array selected when flag is true
   * @param {Uint8Array} b - Array selected when flag is false
   * @return {Uint8Array} Selected array contents
   */
  ctSelect(flag, a, b) {
    const len = a.length;
    const result = new Uint8Array(len);
    const mask = (-(flag | 0)) & 0xff;
    for (let i = 0; i < len; i++) {
